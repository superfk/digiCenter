...
let runningTimeIdicator = document.getElementById('runningTestTime');
let runningTestTimeProg = document.getElementById('runningTestTimeProg');
...
let smallCircleStatusOption = {
    childR: baseRStatus - 25,
    childTxtR: baseRStatus - 50,
    radius: 8,
    textOption: {
        "font-size": "0.8rem",
        "style": 'fill: black',
...
const updateTimer = () => {
    ...
    const timePassed = (currentTime - testStartTime) / 1000; // second
    const remainTime = currentTestApproxTimeInSec - timePassed;
    const progPercentage = remainTime > 0 ? timePassed / currentTestApproxTimeInSec * 100 : 99;
    runningTimeIdicator.innerText = tools.sec2dt(timePassed).substr(11, 8);
    runningTestTimeProg.value = progPercentage;
}
...
function batch_confirmed() {
    ...
    document.getElementById('batchMaxTime').innerHTML = tools.sec2dt(currentTestApproxTimeInSec).substr(11, 8);
}
